...
// ==========================================
// WYCENA V14 — PANEL EDYCJA POD "DODAJ POZYCJĘ"
// ==========================================

let edytowanaPozycjaWycenyId = null;

function znajdzKontenerDodajPozycjeV14() {
    const selectors = [
        "#wycena .form-card",
        "#wycena .side-card",
        ".wycena-form",
    ...
    for (const sel of selectors) {
        ...
        const found = items.find(el => /Dodaj pozycj[eę]/i.test(el.textContent || ""));
        if (found) return found;
    }

    const input = document.getElementById("wycena-usluga");
    return input ? input.closest(".card, .form-card, section, div") : null;
}

function utworzPanelEdycjiPozycjiV14() {
    if (document.getElementById("panel-edycja-pozycji")) return;

    const panel = document.createElement("div");
    panel.id = "panel-edycja-pozycji";
    panel.className = "card edit-position-card";
    ...
    panel.innerHTML = `
        ...
        <div class="form-actions edit-actions">
            <button class="btn btn-primary" type="button" onclick="zapiszEdycjePozycjiV14()">Zapisz zmiany</button>
            <button class="btn btn-secondary" type="button" onclick="anulujEdycjePozycjiV14()">Anuluj</button>
        </div>
    `;

    const addCard = znajdzKontenerDodajPozycjeV14();
    if (addCard && addCard.parentNode) {
        addCard.insertAdjacentElement("afterend", panel);
    } else {
        const wycena = document.getElementById("wycena") || document.body;
        wycena.appendChild(panel);
    }
}

function pokazPanelEdycjiPozycjiV14(id) {
    if (rolaUsera === "guest") {
        alert("Gość nie może edytować wyceny.");
        return;
    }

    utworzPanelEdycjiPozycjiV14();
    ...
    edytowanaPozycjaWycenyId = p.id;

    document.getElementById("edit-pozycja-nazwa").value = p.nazwa || p.name || "";
    document.getElementById("edit-pozycja-jednostka").value = p.jednostka || p.unit || "szt.";
    document.getElementById("edit-pozycja-ilosc").value = Number(p.ilosc ?? p.quantity ?? 1);
    document.getElementById("edit-pozycja-cena").value = Number(p.cenaNetto ?? p.cena_netto ?? p.cena ?? p.price ?? 0);
    document.getElementById("edit-pozycja-vat").value = String(p.vatProcent ?? p.vat ?? p.vat_rate ?? 23);
    document.getElementById("edit-pozycja-uwaga").value = p.uwaga || p.note || "";

    panel.style.display = "block";
    panel.scrollIntoView({ behavior: "smooth", block: "start" });
}

function zapiszEdycjePozycjiV14() {
    if (!edytowanaPozycjaWycenyId) {
        alert("Nie wybrano pozycji do edycji.");
        return;
    }

    const p = wycenaPozycje.find(x => String(x.id) === String(edytowanaPozycjaWycenyId));
    if (!p) {
        alert("Nie znaleziono pozycji.");
        return;
    }
    ...
    const ilosc = Number(String(document.getElementById("edit-pozycja-ilosc").value).replace(",", "."));
    const cena = Number(String(document.getElementById("edit-pozycja-cena").value).replace(",", "."));
    const vat = Number(document.getElementById("edit-pozycja-vat").value);
    ...
    renderujWycene();
    anulujEdycjePozycjiV14();
}

function anulujEdycjePozycjiV14() {
    edytowanaPozycjaWycenyId = null;
    const panel = document.getElementById("panel-edycja-pozycji");
    if (panel) panel.style.display = "none";
}

window.pokazPanelEdycjiPozycjiV14 = pokazPanelEdycjiPozycjiV14;
window.zapiszEdycjePozycjiV14 = zapiszEdycjePozycjiV14;
window.anulujEdycjePozycjiV14 = anulujEdycjePozycjiV14;

// Nadpisujemy renderowanie tabeli: zamiast "Cena" jest "Edycja".
function renderujWycene() {
    utworzPanelEdycjiPozycjiV14();
    ...
    tbody.innerHTML = wycenaPozycje.map(p => {
        ...
        const akcje = rolaUsera !== "guest"
            ? `
                <div class="table-actions">
                    <button class="btn btn-secondary small-btn" onclick="pokazPanelEdycjiPozycjiV14('${esc(p.id)}')">Edycja</button>
                    <button class="btn btn-danger small-btn" onclick="usunPozycjeWyceny('${esc(p.id)}')">Usuń</button>
                </div>
            `
            : "";
...
document.addEventListener("DOMContentLoaded", () => {
    setTimeout(utworzPanelEdycjiPozycjiV14, 400);
});



// ==========================================
// WYCENA V15 — EDYCJA W PANELU + AKCJE W JEDNEJ LINII
// ==========================================

function znajdzKontenerDodajPozycjeV15() {
    const cards = Array.from(document.querySelectorAll(".card, .form-card, .side-card, section, aside, div"));
    const addCard = cards.find(el => {
        const txt = (el.textContent || "").toLowerCase();
        return txt.includes("dodaj pozycj") && txt.includes("rodzaj usługi");
    });
    return addCard || document.getElementById("wycena") || document.body;
}

function utworzPanelEdycjiPozycjiV15() {
    ...
    panel.innerHTML = `
        ...
        <div class="edit-actions">
            <button class="btn btn-primary" type="button" onclick="zapiszEdycjePozycjiV15()">Zapisz zmiany</button>
            <button class="btn btn-secondary" type="button" onclick="anulujEdycjePozycjiV15()">Anuluj</button>
        </div>
    `;

    const addCard = znajdzKontenerDodajPozycjeV15();
    if (addCard && addCard.parentNode && addCard !== document.body) {
        addCard.insertAdjacentElement("afterend", panel);
    } else {
        (document.getElementById("wycena") || document.body).appendChild(panel);
    }

    return panel;
}

function pobierzPozycjeWycenyV15() {
    if (Array.isArray(window.wycenaPozycje)) return window.wycenaPozycje;
    try {
        if (typeof wycenaPozycje !== "undefined" && Array.isArray(wycenaPozycje)) return wycenaPozycje;
    } catch {}
    window.wycenaPozycje = [];
    return window.wycenaPozycje;
}

let edytowanaPozycjaWycenyIdV15 = null;

window.pokazPanelEdycjiPozycjiV15 = function(id) {
    if (typeof rolaUsera !== "undefined" && rolaUsera === "guest") {
        alert("Gość nie może edytować wyceny.");
        return;
    }

    const lista = pobierzPozycjeWycenyV15();
    ...
    const panel = utworzPanelEdycjiPozycjiV15();
    edytowanaPozycjaWycenyIdV15 = p.id;

    const cena = Number(p.cenaNetto ?? p.cena_netto ?? p.cena ?? p.price ?? 0);
    const ilosc = Number(p.ilosc ?? p.quantity ?? 1);
    const vat = Number(p.vatProcent ?? p.vat ?? p.vat_rate ?? 23);

    document.getElementById("edit-pozycja-nazwa").value = p.nazwa || p.name || p.usluga || "";
    document.getElementById("edit-pozycja-jednostka").value = p.jednostka || p.unit || "szt.";
    document.getElementById("edit-pozycja-ilosc").value = Number.isFinite(ilosc) ? ilosc : 1;
    document.getElementById("edit-pozycja-cena").value = Number.isFinite(cena) ? cena : 0;
    document.getElementById("edit-pozycja-vat").value = String(Number.isFinite(vat) ? vat : 23);
    document.getElementById("edit-pozycja-uwaga").value = p.uwaga || p.note || "";
...
window.zapiszEdycjePozycjiV15 = function() {
    const lista = pobierzPozycjeWycenyV15();
    const p = lista.find(x => String(x.id) === String(edytowanaPozycjaWycenyIdV15));
    ...
    if (typeof renderujWycene === "function") renderujWycene();
    if (typeof przeliczWycene === "function") przeliczWycene();

    anulujEdycjePozycjiV15();
};

window.anulujEdycjePozycjiV15 = function() {
    edytowanaPozycjaWycenyIdV15 = null;
    const panel = document.getElementById("panel-edycja-pozycji");
    if (panel) panel.style.display = "none";
};

function dopilnujAkcjiWJednejLiniiV15() {
    document.querySelectorAll("button").forEach(btn => {
        ...
        if (txt === "cena") {
            ...
            // Zamiana starego onclick Cena na panel Edycja, jeśli jest możliwa.
            if (onclick.includes("edytujCenePozycjiWyceny")) {
                const match = onclick.match(/edytujCenePozycjiWyceny\(['"]([^'"]+)['"]\)/);
                if (match) btn.setAttribute("onclick", `pokazPanelEdycjiPozycjiV15('${match[1]}')`);
            }
        }

        if ((btn.textContent || "").trim().toLowerCase() === "edycja") {
            btn.classList.add("btn-edit-inline");
        }

        if ((btn.textContent || "").trim().toLowerCase() === "usuń" || (btn.textContent || "").trim().toLowerCase() === "usun") {
            btn.classList.add("btn-delete-inline");
        }
    });

    document.querySelectorAll(".table-actions").forEach(el => {
        el.classList.add("table-actions-inline");
    });
}

// Ostateczne renderowanie tabeli z przyciskami Edycja/Usuń w jednej linii.
function renderujWycene() {
    utworzPanelEdycjiPozycjiV15();

    const tbody = document.getElementById("tabela-wyceny");
    if (!tbody) return;

    let lista = pobierzPozycjeWycenyV15();
    ...
    tbody.innerHTML = lista.map(p => {
        ...
        return `
            <tr>
                ...
                <td>${ilosc}</td>
                <td><strong>${formatujKwoteV13 ? formatujKwoteV13(cena) : cena.toFixed(2) + " PLN"}</strong></td>
                <td>${formatujKwoteV13 ? formatujKwoteV13(netto) : netto.toFixed(2) + " PLN"}</td>
                <td>${vatProcent}%</td>
                <td>${formatujKwoteV13 ? formatujKwoteV13(brutto) : brutto.toFixed(2) + " PLN"}</td>
                <td>
                    <div class="table-actions table-actions-inline">
                        <button class="btn btn-secondary small-btn btn-edit-inline" onclick="pokazPanelEdycjiPozycjiV15('${esc(p.id)}')">Edycja</button>
                        <button class="btn btn-danger small-btn btn-delete-inline" onclick="usunPozycjeWyceny('${esc(p.id)}')">Usuń</button>
                    </div>
                </td>
            </tr>
        `;
    ...
    if (typeof przeliczWycene === "function") przeliczWycene();
    setTimeout(dopilnujAkcjiWJednejLiniiV15, 20);
}

document.addEventListener("DOMContentLoaded", () => {
    setTimeout(() => {
        utworzPanelEdycjiPozycjiV15();
        dopilnujAkcjiWJednejLiniiV15();
    }, 500);

    const obs = new MutationObserver(() => dopilnujAkcjiWJednejLiniiV15());
...
// ==========================================
// PATCH V18 — stałe okno Edycja dokładnie pod Dodaj pozycję
// ==========================================
(function(){
    let edytowanaPozycjaIdV18 = null;

    function n18(v, fallback = 0) {
        if (typeof v === "number") return Number.isFinite(v) ? v : fallback;
        if (v === null || v === undefined || v === "") return fallback;
    ...
    function norm18(p) {
        const nazwa = p.nazwa || p.name || p.usluga || "Pozycja";
        const jednostka = p.jednostka || p.unit || "szt.";
        const ilosc = n18(p.ilosc ?? p.quantity ?? p.qty, 1);
        const cena = n18(p.cenaNetto ?? p.cena_netto ?? p.cena ?? p.price, 0);
        const vat = n18(p.vatProcent ?? p.vat ?? p.vat_rate, 23);

        return {
            ...p,
            id: p.id || ("poz-" + Date.now() + "-" + Math.random().toString(16).slice(2)),
            nazwa, name: nazwa, usluga: nazwa,
            jednostka, unit: jednostka,
            ilosc, quantity: ilosc,
            cenaNetto: cena, cena_netto: cena, cena: cena, price: cena,
            vatProcent: vat, vat: vat, vat_rate: vat,
            uwaga: p.uwaga || p.note || "",
            note: p.uwaga || p.note || ""
        };
    }

    function findAddCard18() {
        const section = document.getElementById("section-wycena") || document.getElementById("wycena") || document.body;
        ...
        const byTitle = cards.find(card => {
            const header = card.querySelector("h1,h2,h3");
            return header && /dodaj pozycj/i.test(header.textContent || "");
        });
        if (byTitle) return byTitle;

        const any = cards.find(card => /dodaj pozycj/i.test(card.textContent || "") && /rodzaj us/i.test(card.textContent || ""));
        return any || null;
    }

    function buildPanel18() {
        const panel = document.createElement("div");
        panel.id = "panel-edycja-pozycji-v18";
        panel.className = "card edit-position-card-v18";
        panel.innerHTML = `
            <h2><span class="section-marker"></span>Edycja</h2>

            <div id="edit-pozycja-info-v18" class="edit-empty-v18">
                Wybierz pozycję z tabeli przyciskiem <strong>Edycja</strong>.
            </div>

            <div id="edit-pozycja-form-v18" style="display:none;">
                <label>
                    Nazwa usługi
                    <input id="edit-pozycja-nazwa" type="text" placeholder="Nazwa pozycji">
                </label>
                ...
                <div class="edit-actions-v18">
                    <button class="btn btn-primary" type="button" onclick="zapiszEdycjePozycjiV18()">Zapisz zmiany</button>
                    <button class="btn btn-secondary" type="button" onclick="anulujEdycjePozycjiV18()">Anuluj</button>
                </div>
            </div>
        `;
        return panel;
    ...
    function ensurePanel18() {
        // Usuń stare panele, żeby nie było duplikatów i żeby żaden nie siedział po prawej.
        document.querySelectorAll("#panel-edycja-pozycji, #panel-edycja-pozycji-v18").forEach(el => {
            if (el.id !== "panel-edycja-pozycji-v18") el.remove();
        });

        let panel = document.getElementById("panel-edycja-pozycji-v18");
        if (!panel) panel = buildPanel18();

        const addCard = findAddCard18();
        if (addCard && addCard.parentNode) {
            if (panel.parentNode !== addCard.parentNode || panel.previousElementSibling !== addCard) {
                addCard.insertAdjacentElement("afterend", panel);
            }
        }
    ...
    window.pokazPanelEdycjiPozycjiV18 = function(id) {
        const list = getList18().map(norm18);
        setList18(list);
        ...
        const panel = ensurePanel18();
        edytowanaPozycjaIdV18 = p.id;

        const info = document.getElementById("edit-pozycja-info-v18");
        const form = document.getElementById("edit-pozycja-form-v18");
        if (info) info.style.display = "none";
        if (form) form.style.display = "block";

        document.getElementById("edit-pozycja-nazwa").value = p.nazwa;
        document.getElementById("edit-pozycja-jednostka").value = p.jednostka;
        document.getElementById("edit-pozycja-ilosc").value = p.ilosc;
        document.getElementById("edit-pozycja-cena").value = p.cenaNetto;
        document.getElementById("edit-pozycja-vat").value = String(p.vatProcent);
        document.getElementById("edit-pozycja-uwaga").value = p.uwaga || "";

        panel.scrollIntoView({ behavior: "smooth", block: "nearest" });
    };

    window.zapiszEdycjePozycjiV18 = function() {
        if (!edytowanaPozycjaIdV18) {
            alert("Nie wybrano pozycji do edycji.");
            return;
        }

        const list = getList18().map(norm18);
        const p = list.find(x => String(x.id) === String(edytowanaPozycjaIdV18));
        if (!p) {
            alert("Nie znaleziono pozycji.");
            return;
        }
        ...
        const nazwa = document.getElementById("edit-pozycja-nazwa").value.trim();
        const jednostka = document.getElementById("edit-pozycja-jednostka").value;
        const ilosc = n18(document.getElementById("edit-pozycja-ilosc").value, NaN);
        const cena = n18(document.getElementById("edit-pozycja-cena").value, NaN);
        const vat = n18(document.getElementById("edit-pozycja-vat").value, 23);
        const uwaga = document.getElementById("edit-pozycja-uwaga").value.trim();
        ...
        p.nazwa = nazwa; p.name = nazwa; p.usluga = nazwa;
        p.jednostka = jednostka; p.unit = jednostka;
        p.ilosc = ilosc; p.quantity = ilosc;
        p.cenaNetto = cena; p.cena_netto = cena; p.cena = cena; p.price = cena;
        p.vatProcent = vat; p.vat = vat; p.vat_rate = vat;
        p.uwaga = uwaga; p.note = uwaga;

        setList18(list);
        renderujWycene();
        anulujEdycjePozycjiV18();
    };

    window.anulujEdycjePozycjiV18 = function() {
        edytowanaPozycjaIdV18 = null;
        ensurePanel18();
        const info = document.getElementById("edit-pozycja-info-v18");
        const form = document.getElementById("edit-pozycja-form-v18");
    ...
    // Zgodność ze starszymi nazwami funkcji z poprzednich wersji.
    window.pokazPanelEdycjiPozycjiV17 = window.pokazPanelEdycjiPozycjiV18;
    window.pokazPanelEdycjiPozycjiV15 = window.pokazPanelEdycjiPozycjiV18;
    window.zapiszEdycjePozycjiV17 = window.zapiszEdycjePozycjiV18;
    window.zapiszEdycjePozycjiV15 = window.zapiszEdycjePozycjiV18;
    window.anulujEdycjePozycjiV17 = window.anulujEdycjePozycjiV18;
    window.anulujEdycjePozycjiV15 = window.anulujEdycjePozycjiV18;
    ...
    renderujWycene = function() {
        ...
        tbody.innerHTML = list.map(p => {
            const netto = p.ilosc * p.cenaNetto;
            const vat = Number(p.vatProcent || 23);
            ...
            return `
                <tr>
                    ...
                    <td>${esc(p.jednostka)}</td>
                    <td>${p.ilosc}</td>
                    <td>${money18(p.cenaNetto)}</td>
                    <td>${money18(netto)}</td>
                    <td>${vat}%</td>
                    <td>${money18(brutto)}</td>
                    <td>
                        <div class="wycena-actions-v18">
                            <button class="btn btn-secondary small-btn" onclick="pokazPanelEdycjiPozycjiV18('${esc(String(p.id))}')">Edycja</button>
                            <button class="btn btn-danger small-btn" onclick="usunPozycjeWyceny('${esc(String(p.id))}')">Usuń</button>
                        </div>
                    </td>
                </tr>
    ...
    window.usunPozycjeWyceny = function(id) {
        const list = getList18().filter(p => String(p.id) !== String(id));
        setList18(list);
        renderujWycene();
    };

    const oldWczytaj = window.wczytajKosztorys;
    if (typeof oldWczytaj === "function") {
        window.wczytajKosztorys = function(id) {
            oldWczytaj(id);
            try {
                if (typeof wycenaPozycje !== "undefined" && Array.isArray(wycenaPozycje)) {
                    setList18(wycenaPozycje.map(norm18));
                }
            } catch(e) {}
            setTimeout(() => {
                ensurePanel18();
