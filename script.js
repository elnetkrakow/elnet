...
// ==========================================
// PATCH V20 — minimalna Edycja pod Dodaj pozycję, bez zmiany układu Wyceny
// ==========================================
(function(){
    let edytowanaPozycjaIdV20 = null;

    function num20(v, fallback = 0) {
    ...
    function norm20(p) {
        const nazwa = p.nazwa || p.name || p.usluga || "Pozycja";
        const jednostka = p.jednostka || p.unit || "szt.";
        const ilosc = num20(p.ilosc ?? p.quantity ?? p.qty, 1);
        const cena = num20(p.cenaNetto ?? p.cena_netto ?? p.cena ?? p.price, 0);
        const vat = num20(p.vatProcent ?? p.vat ?? p.vat_rate, 23);
    ...
    function findAddCard20() {
        ...
        return cards.find(card => {
            ...
            return h && /dodaj pozycj/i.test(h.textContent || "");
        }) || null;
    }

    function createPanel20() {
        const panel = document.createElement("div");
        panel.id = "panel-edycja-pozycji-v20";
        panel.className = "card panel-edycja-pozycji-v20";
        panel.innerHTML = `
            <h2><span class="section-marker"></span>Edycja</h2>

            <div id="edit-info-v20" class="edit-info-v20">
                Wybierz pozycję z tabeli przyciskiem <strong>Edycja</strong>.
            </div>

            <div id="edit-form-v20" style="display:none;">
                <label>
                    Nazwa usługi
                    <input id="edit-pozycja-nazwa-v20" type="text" placeholder="Nazwa pozycji">
                </label>

                <label>
                    Jednostka
                    <select id="edit-pozycja-jednostka-v20">
                ...
                <label>
                    Ilość
                    <input id="edit-pozycja-ilosc-v20" type="number" step="0.01" min="0" placeholder="np. 10">
                </label>

                <label>
                    Cena netto PLN
                    <input id="edit-pozycja-cena-v20" type="number" step="0.01" min="0" placeholder="np. 120">
                </label>

                <label>
                    Stawka VAT
                    <select id="edit-pozycja-vat-v20">
                ...
                <label>
                    Uwagi
                    <input id="edit-pozycja-uwaga-v20" type="text" placeholder="Opcjonalnie">
                </label>

                <div class="edit-actions-v20">
                    <button class="btn btn-primary" type="button" onclick="zapiszEdycjePozycjiV20()">Zapisz zmiany</button>
                    <button class="btn btn-secondary" type="button" onclick="anulujEdycjePozycjiV20()">Anuluj</button>
                </div>
            </div>
        `;
        return panel;
    ...
    function ensurePanel20() {
        let panel = document.getElementById("panel-edycja-pozycji-v20");
        if (!panel) panel = createPanel20();

        const addCard = findAddCard20();
    ...
    window.pokazEdycjePozycjiV20 = function(id) {
        const lista = getList20().map(norm20);
        setList20(lista);
        ...
        const panel = ensurePanel20();
        edytowanaPozycjaIdV20 = p.id;

        document.getElementById("edit-info-v20").style.display = "none";
        document.getElementById("edit-form-v20").style.display = "block";

        document.getElementById("edit-pozycja-nazwa-v20").value = p.nazwa;
        document.getElementById("edit-pozycja-jednostka-v20").value = p.jednostka;
        document.getElementById("edit-pozycja-ilosc-v20").value = p.ilosc;
        document.getElementById("edit-pozycja-cena-v20").value = p.cenaNetto;
        document.getElementById("edit-pozycja-vat-v20").value = String(p.vatProcent);
        document.getElementById("edit-pozycja-uwaga-v20").value = p.uwaga || "";

        panel.scrollIntoView({ behavior: "smooth", block: "nearest" });
    };

    window.zapiszEdycjePozycjiV20 = function() {
        if (!edytowanaPozycjaIdV20) {
            alert("Nie wybrano pozycji do edycji.");
            return;
        }

        const lista = getList20().map(norm20);
        const p = lista.find(x => String(x.id) === String(edytowanaPozycjaIdV20));
        if (!p) {
            alert("Nie znaleziono pozycji.");
            return;
        }

        const nazwa = document.getElementById("edit-pozycja-nazwa-v20").value.trim();
        const jednostka = document.getElementById("edit-pozycja-jednostka-v20").value;
        const ilosc = num20(document.getElementById("edit-pozycja-ilosc-v20").value, NaN);
        const cena = num20(document.getElementById("edit-pozycja-cena-v20").value, NaN);
        const vat = num20(document.getElementById("edit-pozycja-vat-v20").value, 23);
        const uwaga = document.getElementById("edit-pozycja-uwaga-v20").value.trim();
        ...
        setList20(lista);
        renderujWycene();
        anulujEdycjePozycjiV20();
    };

    window.anulujEdycjePozycjiV20 = function() {
        edytowanaPozycjaIdV20 = null;
        const panel = ensurePanel20();
        const info = document.getElementById("edit-info-v20");
        const form = document.getElementById("edit-form-v20");
        if (info) info.style.display = "block";
        if (form) form.style.display = "none";
    };

    // Render tylko tabeli — bez zmiany układu strony.
    const renderWycenyV20 = function() {
        const tbody = document.getElementById("tabela-wyceny");
        if (!tbody) return;
        ...
        tbody.innerHTML = lista.map(p => {
            ...
            return `
                <tr>
                    ...
                    <td>${esc(p.jednostka)}</td>
                    <td>${p.ilosc}</td>
                    <td>${money20(p.cenaNetto)}</td>
                    <td>${money20(netto)}</td>
                    <td>${vat}%</td>
                    <td>${money20(brutto)}</td>
                    <td>
                        <div class="wycena-actions-v20">
                            <button class="btn btn-secondary small-btn" onclick="pokazEdycjePozycjiV20('${esc(String(p.id))}')">Edycja</button>
    ...
    renderujWycene = renderWycenyV20;

    window.usunPozycjeWyceny = function(id) {
        const lista = getList20().filter(p => String(p.id) !== String(id));
        setList20(lista);
        renderujWycene();
    };

    // Pilnuj, żeby po edycji kosztorysu lista pozostała w oryginalnej zmiennej.
