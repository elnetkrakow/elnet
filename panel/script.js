...
window.usunTermin = async function(id) {
    if (rolaUsera === "guest") {
        alert("Tylko zalogowany u�ytkownik mo�e usuwa� terminy.");
        return;
    }
    ...
    if (investmentId && inwestycja) {
        if (!confirm("Ten wpis jest po��czony z inwestycj�. Czy usun�� tylko wpis z Terminarza i od��czy� inwestycj�?")) return;
    } else if (!confirm("Usun�� termin?")) {
        return;
    }
    ...
    try {
        ...
        zapiszLog("Terminarz", "Usuni�to termin", id);
    } catch (err) {
        console.error("B��d usuwania terminarza:", err);
        alert("Nie uda�o si� usun�� terminu.");
    }
};
...
function renderujTerminarz() {
    ...
    tbody.innerHTML = lista.map(item => {
        ...
        const investmentButton = orphanInvestmentLink
            ? `<button class="btn btn-danger small-btn" onclick="usunTermin('${esc(item.id)}')">Usu� wpis z Terminarza</button> <button class="btn btn-secondary small-btn" onclick="odlaczTermin('${esc(item.id)}')">Od��cz powi�zanie</button> <button class="btn btn-secondary small-btn" onclick="polaczTerminZInnaInwestycja('${esc(item.id)}')">Po��cz z inn� inwestycj�</button>`
            : investmentId
                ? `<button class="btn btn-secondary small-btn" onclick="przejdzDoInwestycjiZTerminu('${esc(item.id)}')">Przejd� do inwestycji</button>`
                : itemType === "Inwestycja"
                    ? `<button class="btn btn-secondary small-btn" onclick="obsluzTerminInwestycji('${esc(item.id)}')">Obs�u� inwestycj�</button>`
                    : '';
...
window.odlaczTermin = async function(id) {
    const termin = terminarz.find(t => String(t.id) === String(id));
    if (!termin) return;
    if (!confirm("Od��czy� ten wpis od inwestycji?")) return;
    try {
        ...
        alert("Nie uda�o si� od��czy� wpisu od inwestycji.");
    }
};
...
window.polaczTerminZInnaInwestycja = async function(id) {
    ...
    try {
        ...
        alert("Nie uda�o si� po��czy� wpisu z inwestycj�.");
    }
};
...
window.przejdzDoInwestycjiZTerminu = async function(id) {
    ...
    const wybor = prompt("Ta inwestycja zosta�a usuni�ta albo nie istnieje.\n1 - Usu� wpis z Terminarza\n2 - Od��cz wpis od inwestycji\n3 - Anuluj", "3");
    try {
        if (wybor === "1") {
            await usunTerminZBazy(id);
        ...
        alert("Nie uda�o si� obs�u�y� powi�zania z usuni�t� inwestycj�.");
    }
};
...
window.dodajInwestycjeDoTerminarza = async function(id) {
    ...
    try {
        ...
        alert("Nie uda�o si� doda� inwestycji do Terminarza. Szczeg�y b��du s� w konsoli F12.");
    }
};
...
window.usunInwestycje = async function(id) {
    if (rolaUsera !== "admin") {
        alert("Tylko administrator mo�e usuwa� inwestycje.");
        return;
    }

    const inwestycja = inwestycje.find(i => String(i.id) === String(id));
    if (!inwestycja) {
        alert("Nie znaleziono inwestycji do usuni�cia.");
        return;
    }
    ...
    if (linkedEventId) {
        const wybor = prompt("Ta inwestycja jest po��czona z Terminarzem. Co zrobi� z wpisem w Terminarzu?\n1 - Usu� r�wnie� wpis z Terminarza\n2 - Zostaw wpis w Terminarzu, ale od��cz od inwestycji\n3 - Anuluj", "3");
        if (wybor === "1") {
    ...
    } else if (!confirm("Usun�� inwestycj� razem z jej zaliczkami i kosztami?")) {
        return;
    }
    ...
    try {
        ...
        renderujKalendarzTerminarza();
        renderujPulpit();
        zapiszLog("Inwestycje", "Usuni�to inwestycj�", id);
        ...
        alert("Nie uda�o si� usun�� inwestycji.");
    }
}
